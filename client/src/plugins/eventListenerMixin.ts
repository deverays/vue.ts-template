...
export default {
    click: function (items: Array<{ id: string; ref: Ref<boolean> | any }>) {
        document.addEventListener('click', (event) => {
            ...
            items.forEach(({ id, ref }) => {
                ...
                if (ref.value && !targetElement?.contains(target) && !targetElementButton?.contains(target))
                    ref.value = false;
            });
        });
    },
    mousemove: function (items: Array<{ id: string; ref: Ref<boolean> }>) {
